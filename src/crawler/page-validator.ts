  ...
  static async isValidHtmlPage(page: Page): Promise<boolean> {
    try {
      ...
      if (!structure.hasHtml || !structure.hasHead || !structure.hasBody) {
        console.log(`無効なHTML構造 (${page.url()}): html=${structure.hasHtml}, head=${structure.hasHead}, body=${structure.hasBody}`);
        return false;
      }
  ...
  static async hasContent(page: Page): Promise<boolean> {
    try {
      const contentInfo = await page.evaluate(() => {
        ...
        return {
          // 主要なコンテンツ要素が存在するか
          hasMainElement: !!document.querySelector('main, article, .content, #content, [role="main"]'),
          // テキストコンテンツの量をチェック
      ...
      // コンテンツがほぼないページは除外
      if (contentInfo.textLength < 50 && 
          !contentInfo.hasMainElement && 
          contentInfo.linkCount < 2 && 
          contentInfo.buttonCount < 2) {
        console.log(`コンテンツが不十分 (${page.url()}): テキスト長=${contentInfo.textLength}, リンク数=${contentInfo.linkCount}`);
        return false;
      }
